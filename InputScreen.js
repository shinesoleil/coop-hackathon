...
class InputScreen extends React.Component {
  ...
  calculateFBI = () => {
    const res = (parseInt(this.state.firstNum * 6)
  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        <View style={{ flex: 1, height: 1000, flexDirection: "row", paddingLeft: 10, paddingRight: 10}}>
          <View style={{flex: 1}}>
        ...
        <Text style={{ padding: 10, fontSize: 20 }}>
          FBI（科级耐污指数）：{this.calculateFBI()}
        </Text>

        <Text style={{ padding: 10, fontSize: 20 }}>
          水质综合评价：{this.translateStatus(this.calculateFBI())}
        </Text>
      </View>
    )
  }
